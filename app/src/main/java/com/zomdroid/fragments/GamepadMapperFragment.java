...
/**
 * Fragment for step-by-step mapping of 10 gamepad buttons (no D-Pad).
 * User maps each button in order. Mapping is saved at the end or reset to default.
 */
    private static final int STEP_COUNT = 10; // 10 buttons
    ...
    private void resetMapping() {
        ...
        // Copy all except GUIDE (8) and D-Pad (not mapped)
        for (int i = 0, j = 0; i < defaultMap.length && j < STEP_COUNT; i++) {
            if (i == 8) continue; // skip GUIDE
            if (i >= 11) continue; // skip D-Pad
            mapping[j++] = defaultMap[i];
        }
    ...
    private int[] fullMappingFromCurrent() {
        int[] fullMapping = new int[GamepadManager.GAMEPAD_BUTTON_COUNT];
        for (int i = 0, j = 0; i < fullMapping.length; i++) {
            if (i == 8) {
                fullMapping[i] = GamepadManager.getCurrentMapping()[8]; // keep default GUIDE
            } else if (i < 8) {
                fullMapping[i] = mapping[j++];
            } else if (i > 8 && i < 11) {
                fullMapping[i] = mapping[j++];
            } else {
                // D-Pad: keep from current mapping
                fullMapping[i] = GamepadManager.getCurrentMapping()[i];
            }
        }
        return fullMapping;
    }
